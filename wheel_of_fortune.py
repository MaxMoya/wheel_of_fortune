...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        promp = f"""{self.name} has ${self.prizeMoney}
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        if self.prizeMoney < VOWEL_COST:
            for v in VOWELS:
                if v in possible_letters:
                    possible_letters.remove(v)
                
        '''if self.prizeMoney >= VOWEL_COST:
            possible_letters.append(VOWELS)
        else:
            pass
    ...
    def getMove(self, category, obscuredPhrase, guessed):
            ...
            # If there are no possible letters, pass
            if not possible_letters:
                return 'pass'

            # Decide on good or bad move
            if self.smartCoinFlip():
                # Good move: choose most frequent possible letter
                for letter in self.SORTED_FREQUENCIES[::-1]: # Iterate from most frequent to least
                    if letter in possible_letters:
                        return letter
            else:
                # Bad move: choose a random possible letter
